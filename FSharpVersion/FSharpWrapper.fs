...
module FloatDistribution =
    ...
    type VectorGaussian = Microsoft.ML.Probabilistic.Distributions.VectorGaussian
    type Beta = Microsoft.ML.Probabilistic.Distributions.Beta
    type Dirichlet = Microsoft.ML.Probabilistic.Distributions.Dirichlet
    ...
    type VariableVectorGaussianArrayOfArrayOfArray =
        VariableArray<VariableVectorGaussianArrayOfArray, VectorGaussian [] [] []>
...
module Variable =
    ...
    let IfBlock (test: Variable<bool>) (f1IfTest: Variable<bool> -> unit) (f2NotTest: Variable<bool> -> unit) =
    ...
    let AssignJagged1DVariableArray
        (v: VariableArray<VariableArray<'a>, 'a [] []>)
        (r: Range)
        (s: Range)
        (f: Range -> Range -> Variable<'a>)
        =
        v.[r].[s] <- f r s
        v
...
module Inference =
    ...
    let InferGaussianArray (ie: InferenceEngine, m: VariableArray<float>) = ie.Infer<Gaussian []>(m)

    ///Infer a VariableArray<Vector> as a.NET array of VectorGaussian distributions
    let InferVectorGaussianArray (ie: InferenceEngine, m: VariableArray<Vector>) = ie.Infer<VectorGaussian []>(m)

    ///Infer a VariableArray<Vector> as a.NET array of Dirichlet distributions
    let InferDirichletArray (ie: InferenceEngine, m: VariableArray<Vector>) = ie.Infer<Dirichlet []>(m)

    ///Infer a VariableArray<PositiveDefiniteMatrix> as a.NET array of Wishart distributions
    let InferWishartArray (ie: InferenceEngine, m: VariableArray<PositiveDefiniteMatrix>) = ie.Infer<Wishart []>(m)

    ///Infer a VariableArray<float> as a.NET array of Beta distributions
    let InferBetaArray (ie: InferenceEngine, m: VariableArray<float>) = ie.Infer<Beta []>(m)

    ///Infer a VariableArray<float> as a.NET array of Gamma distributions
    let InferGammaArray (ie: InferenceEngine, m: VariableArray<float>) = ie.Infer<Gamma []>(m)

    //========================================================================
    //=================== Inference for int arrays ===========================
    //========================================================================
    ///Infer a VariableArray<int> as a.NET array of Discrete distributions
    let InferDiscreteArray (ie: InferenceEngine, m: VariableArray<'a>) = ie.Infer<Discrete []>(m)

    ///Infer a VariableArray<int> as a.NET array of Poisson distributions
    let InferPoissonArray (ie: InferenceEngine, m: VariableArray<'a>) = ie.Infer<Poisson []>(m)

    //========================================================================
    //=================== Inference for bool arrays ==========================
    //========================================================================
    ///Infer a VariableArray<bool> as a.NET array of Bernoulli distributions
    let InferBernoulliArray (ie: InferenceEngine, m: VariableArray<bool>) = ie.Infer<Bernoulli []>(m)
...
module Factors =
    ...
    let createDelegate x =
        ...
        let argTypes =
            argInfo |> Array.map (fun pi -> pi.ParameterType)
        ...
        let methodTyp =
            assembly.GetType(methodString).MakeGenericType(a)

        let d =
            System.Delegate.CreateDelegate(methodTyp, info)
