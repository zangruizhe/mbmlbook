...
public class WetGlassSprinklerRainModel
{
    ...
    // Inference engine
    public InferenceEngine Engine = new();
    ...
    public WetGlassSprinklerRainModel()
    {
        ...
        var N = new Range(NumberOfExamples).Named("N");

        var C = new Range(CloudyStateCount).Named("C");
        var S = new Range(SprinklerStateCount).Named("S");
        var R = new Range(RainStateCount).Named("R");
        var W = new Range(WetGrassStateCount).Named("W");
        ...
        int[] wetgrass)
    {
        ...
        var probCloudyPrior = Dirichlet.Uniform(CloudyStateCount);
        Dirichlet[] cptSprinklerPrior =
            Enumerable.Repeat(Dirichlet.Uniform(SprinklerStateCount), CloudyStateCount).ToArray();
        ...
        var cptWetGrassPrior = Enumerable.Repeat(dirUnifArray, SprinklerStateCount).ToArray();

        LearnParameters(cloudy, sprinkler, rain, wetgrass, probCloudyPrior, cptSprinklerPrior, cptRainPrior,
            cptWetGrassPrior);
    }
    ...
    /// <returns>Probability that it has rained</returns>
    public double ProbRainWithVectorPrior(
        int? cloudy,
        int? sprinkler,
        int? wet,
        Vector probCloudy,
        ...
        Vector[][] cptWetGrass)
    {
        ...
        for (var i = 0; i < 4; i++) sample[i] = new int[numData];

        for (var j = 0; j < numData; j++)
        {
            var cloudy = Discrete.Sample(probCloudy);
            var sprinkler = Discrete.Sample(cptSprinkler[cloudy]);
            var rain = Discrete.Sample(cptRain[cloudy]);
            var wetGrass = Discrete.Sample(cptWetGrass[sprinkler][rain]);
            sample[0][j] = cloudy;
...
public class WetGrassSprinklerRain
{
    public void Run()
    {
        ...
        // Create a new model
        var model = new WetGlassSprinklerRainModel();
        ...
        var probCloudy = Vector.FromArray(0.5, 0.5);
        var cptSprinkler = new Vector[]
            {Vector.FromArray(0.1, 0.9) /* cloudy */, Vector.FromArray(0.5, 0.5) /* not cloudy */};
        var cptRain = new Vector[]
            {Vector.FromArray(0.8, 0.2) /* cloudy */, Vector.FromArray(0.2, 0.8) /* not cloudy */};
        var cptWetGrass = new Vector[][]
        {
            new Vector[]
                {Vector.FromArray(0.99, 0.01) /* rain */, Vector.FromArray(0.9, 0.1) /* not rain */}, // Sprinkler
            new Vector[]
                {Vector.FromArray(0.9, 0.1) /* rain */, Vector.FromArray(0.0, 1.0) /* not rain */} // Not sprinkler
        };

        var probRainGivenWetGrass = model.ProbRainWithVectorPrior(null, null, 0, probCloudy, cptSprinkler, cptRain, cptWetGrass);
        var probRainGivenWetGrassNotCloudy = model.ProbRainWithVectorPrior(1, null, 0, probCloudy, cptSprinkler, cptRain, cptWetGrass);
        ...
        var sample = WetGlassSprinklerRainModel.Sample(1000, probCloudy, cptSprinkler, cptRain, cptWetGrass);
        ...
        Console.WriteLine("Prob. Cloudy:                              Ground truth: {0:0.00}, Inferred: {1:0.00}", 0.5,
            model.ProbCloudyPosterior.GetMean()[0]);
        Console.WriteLine("Prob. Sprinkler | Cloudy:                  Ground truth: {0:0.00}, Inferred: {1:0.00}", 0.1,
            model.CPTSprinklerPosterior[0].GetMean()[0]);
        Console.WriteLine("Prob. Sprinkler | Not Cloudy:              Ground truth: {0:0.00}, Inferred: {1:0.00}", 0.5,
            model.CPTSprinklerPosterior[1].GetMean()[0]);
        Console.WriteLine("Prob. Rain      | Cloudy:                  Ground truth: {0:0.00}, Inferred: {1:0.00}", 0.8,
            model.CPTRainPosterior[0].GetMean()[0]);
        Console.WriteLine("Prob. Rain      | Not Cloudy:              Ground truth: {0:0.00}, Inferred: {1:0.00}", 0.2,
            model.CPTRainPosterior[1].GetMean()[0]);
        Console.WriteLine("Prob. Wet Grass | Sprinkler, Rain:         Ground truth: {0:0.00}, Inferred: {1:0.00}", 0.99,
            model.CPTWetGrassPosterior[0][0].GetMean()[0]);
        Console.WriteLine("Prob. Wet Grass | Sprinkler, Not Rain      Ground truth: {0:0.00}, Inferred: {1:0.00}", 0.9,
            model.CPTWetGrassPosterior[0][1].GetMean()[0]);
        Console.WriteLine("Prob. Wet Grass | Not Sprinkler, Rain:     Ground truth: {0:0.00}, Inferred: {1:0.00}", 0.9,
            model.CPTWetGrassPosterior[1][0].GetMean()[0]);
        Console.WriteLine("Prob. Wet Grass | Not Sprinkler, Not Rain: Ground truth: {0:0.00}, Inferred: {1:0.00}", 0.0,
            model.CPTWetGrassPosterior[1][1].GetMean()[0]);
        ...
        Console.WriteLine("**********************************************");
        var probRainGivenWetGrass1 = model.ProbRain(null, null, 0, model.ProbCloudyPosterior,
            model.CPTSprinklerPosterior, model.CPTRainPosterior, model.CPTWetGrassPosterior);
        var probRainGivenWetGrassNotCloudy1 = model.ProbRain(1, null, 0, model.ProbCloudyPosterior,
            model.CPTSprinklerPosterior, model.CPTRainPosterior, model.CPTWetGrassPosterior);
